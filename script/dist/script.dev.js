...
function help_sound() {
  setTimeout(function () {
    var mus = document.getElementById('notifypop');
    mus.play();
...
function add_to_cart() {
  document.querySelectorAll('a[data-smell]').forEach(function (elem) {
...
function post_render(calc_item, id_brand, id_category) {
  ...
  var smells = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 'smells';
  ...
  $(document).ajaxComplete(function () {
    add_to_cart();
    var elem = $('div[data-length]').attr('data-length');
    var item_count = document.querySelectorAll('.catalog-item').length;
...
$('a[data-load="download"]').click(function () {
  var calc_item = document.querySelectorAll('.catalog-item').length;
...
$('body').click(function (e) {
  if (e.target.classList.contains('brands-button') && $('.brands-list').children().length == 0) {
    ...
    $(document).ajaxComplete(function () {
      document.querySelectorAll('.brands-list-item').forEach(function (elem) {
        $(elem).click(function () {
          document.querySelectorAll('.brands-list-item').forEach(function (element) {
          ...
          var item_active = $('.brands-list-item-active');
...
document.querySelector('.flex-style').querySelectorAll('.category-item').forEach(function (elem) {
  $(elem).click(function () {
    $('input[name]').val('');
...
$('input[name="search"]').on('input', function () {
  if ($(this).val() != '#') {
    ...
    $('.catalog-categories')[0].querySelectorAll('.category-item').forEach(function (elem) {
    ...
    $(document).ajaxComplete(function () {
      ...
      document.querySelectorAll('div[data-tracking]').forEach(function (elem) {
        elem.querySelectorAll('.cat-name').forEach(function (element) {
          var result = element.innerText.replace(new RegExp($(_this).val(), 'i'), '<span class="search">' + $(_this).val() + '</span>');
          element.innerHTML = result;
        });
        elem.querySelector('.cat-value').innerHTML = elem.innerText.replace(new RegExp($(_this).val(), 'i'), '<span class="search">' + $(_this).val() + '</span>');
      });
      $('.catalog-body').show('fast', 'linear');
    });
  }
});
$('div[data-sort]').click(function () {
...
function sum() {
  var sum = 0;
  document.querySelectorAll('span[data-price]').forEach(function (elem) {
    sum += parseInt(elem.innerHTML);
  });
  $('.sum-result').html(sum + '₽');
}

$('div[data-widget="cart"]').click(function () {
  ...
  $(document).ajaxComplete(function () {
    document.querySelectorAll('.order-img').forEach(function (elem) {
    ...
    $('i[data-count="plus"]').click(function (event) {
      ...
      cart.items.forEach(function (elem) {
        if ($(_this2).parents().eq(2).attr('data-id') == elem.id) {
          if ($(_this2).parent().find('input').attr('min') == '10') {
    ...
    $('i[data-count="minus"]').click(function (event) {
      ...
      if ($(this).parent().find('input[name="count"]').val() > 5) {
        ...
        cart.items.forEach(function (elem) {
          if ($(_this3).parents().eq(2).attr('data-id') == elem.id) {
            if ($(_this3).parent().find('input').attr('min') == '10') {
              elem.price -= elem.initialPrice / 2;
            } else {
              elem.price -= elem.initialPrice;
            }
    ...
    $('.delete').click(function (event) {
      event.stopImmediatePropagation();
      var item_index = cart.items.indexOf($(this).parent().attr('data-id'));
...
$('div[data-widget="chat"]').click(function () {
  $('.chat-form').toggle('fast', 'linear');
});
$('div[data-cat="boxes"] ').click(function () {
...
$('div[data-cat="smells"]').click(function () {
